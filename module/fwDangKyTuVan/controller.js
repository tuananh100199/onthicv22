module.exports = app => {
    ...
    app.get('/api/dang-ky-tu-van/page/:pageNumber/:pageSize', app.permission.check('DangKyTuVan:read'), (req, res) => {
        ...
        app.modelDangKyTuVan.getPage(pageNumber, pageSize, {}, (error, page) => {
            page.list = page.list.map(item => app.clone(item, { message: '' }));
            res.send({ error, page });
        });
    });

    app.get('/api/dang-ky-tu-van/all', app.permission.check('DangKyTuVan:read'), (req, res) => app.modelDangKyTuVan.getAll((error, items) => res.send({ error, items })));

    app.get('/api/dang-ky-tu-van/unread', app.permission.check('DangKyTuVan:read'), (req, res) => app.modelDangKyTuVan.getUnread((error, items) => res.send({ error, items })));

    app.get('/api/dang-ky-tu-van/item/:_id', app.permission.check('DangKyTuVan:write'), (req, res) => app.modelDangKyTuVan.read(req.params._id, (error, item) => {
        if (item) app.io.emit('DangKyTuVan-changed', item);
        res.send({ error, item });
    }));

    app.delete('/api/dang-ky-tu-van', app.permission.check('DangKyTuVan:write'), (req, res) => app.modelDangKyTuVan.delete(req.body._id, error => res.send({ error })));


    // Home -----------------------------------------------------------------------------------------------------------------------------------------
    app.post('/api/dang-ky-tu-van', (req, res) => app.modelDangKyTuVan.create(req.bodyDangKyTuVan, (error, item) => {
        if (item) {
