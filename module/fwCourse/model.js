        ...
        read: (condition, done) => model.find(condition).populate('categories').exec((error, items) => {
            if (error) {
                ...
                items[0].save((error, item) => {
                    app.io.emit('course:item-view-changed', item._id, item.view);
                    done(error, item);
                });
            }
        }),
        readById: (_id, done) => app.model.course.read({ _id, active: true }, done),
        readByLink: (link, done) => app.model.course.read({ link, active: true }, done),

        update: (_id, changes, done) => model.findOneAndUpdate({ _id }, { $set: changes }, { new: true }, done),

        swapPriority: (_id, isMoveUp, done) => model.findById(_id, (error, item1) => {
            if (error || item1 === null) {
                done('Invalid course Id!');
            } else {
