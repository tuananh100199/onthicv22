...
export default class AdminPage extends React.Component {
    getPermission = (prefix, listPermissions = ['read', 'write', 'delete']) => {
        const permission = {},
    ...
    parentRender = ({ icon, title, breadcrumb, content, onCreate }) => {
        return (
            <main className='app-content'>
                <div className='app-title'>
                    <h1><i className={icon} />{title}</h1>
                    <ul className='app-breadcrumb breadcrumb'>
                        <Link to='/user'><i className='fa fa-home fa-lg' /></Link>{breadcrumb}
                    </ul>
                </div>
                {content}
                {onCreate ?
