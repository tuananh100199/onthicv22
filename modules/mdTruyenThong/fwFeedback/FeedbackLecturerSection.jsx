...
import { createFeedbackByLecturer, getFeedbackPageByLecturer } from './redux';
import { FormRichTextBox } from 'view/component/AdminPage';
import Pagination from 'view/component/Pagination';

class FeedbackSection extends React.Component {
    componentDidMount() {
        const { _refId } = this.props;
        this.props.getFeedbackPageByLecturer(1, 50, { _refId });
    }
    ...
    sendFeedback = (e) => {
        ...
        if (content == '') {
            ...
            this.props.createFeedbackByLecturer(newData, () => {
    ...
    render() {
        ...
        return <>
            ...
            <Pagination pageCondition={pageCondition} pageNumber={pageNumber} pageSize={pageSize} pageTotal={pageTotal} totalItem={totalItem}
                getPage={this.props.getFeedbackPageByLecturer} style={{ ...(this.props.type != 'system' && { left: 320 }) }} />
        </>;
    }
}

const mapStateToProps = state => ({ system: state.system, feedback: state.communication.feedback });
const mapActionsToProps = { createFeedbackByLecturer, getFeedbackPageByLecturer };
