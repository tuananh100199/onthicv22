    ...
    app.model.forumMessage = {
        ...
        getPage: (pageNumber, pageSize, condition, done) => model.countDocuments(condition, (error, totalItem) => {
            if (error) {
                ...
                model.find(condition).populate('user', 'firstname lastname').sort({ createdDate: -1 }).skip(skipNumber).limit(result.pageSize).exec((error, list) => {
                    result.list = list;
                    done(error, result);
                });
            }
        ...
        update: (_id, changes, done) => {
            model.findById(_id, (error, item) => {
                if (error || item == null) {
                    ...
                    app.model.forum.update(item.forum, {}, error => {
                        error ? done(error) : model.findOneAndUpdate({ _id }, changes, { new: true }).populate('user', 'firstname lastname').exec(done);
                    });
                }
            });
        },
        ...
        delete: (_id, done) => model.findById(_id, (error, item) => {
            if (error || item == null) {
                ...
                app.model.forum.update(item.forum, {}, error => {
                    error ? done(error) : item.remove(done);
                });
            }
        }),

        deleteForum: (forum, done) => {
            model.remove({ forum }, done);
        },
