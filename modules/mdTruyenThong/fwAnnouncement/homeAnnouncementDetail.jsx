...
import { getAnnouncementByUser } from './redux';
...
class AnnouncementDetail extends React.Component {
    ...
    componentDidMount() {
        let url = window.location.pathname,
            params = T.routeMatcher(url.startsWith('/thong-bao/') ? '/thong-bao/:link' : '/announcement/:id').parse(url);
        this.setState({ _id: params.id, link: params.link });
        console.log(params.id,params.link);
        this.props.getAnnouncementByUser(params.id, params.link, () => $(this.background).parallax());
    }
    ...
    componentDidUpdate(prevProps) {
        ...
        if (prevProps.location.pathname != window.location.pathname) {
            ...
            this.setState({ _id: params.id, link: params.link });
            $('.parallax-mirror').length != 0 && $(this.background).parallax('destroy');
            this.props.getAnnouncementByUser(params.id, params.link, (data) => {
                if (data.item) {
    ...
    render() {
        const item = this.props.announcement && this.props.announcement.userNews ? this.props.announcement.userNews : null;
        if (item == null) {
            return <p>...</p>;
        } else {
            return <>
...
const mapStateToProps = state => ({ announcement: state.communication.announcement });
const mapActionsToProps = { getAnnouncementByUser };
export default connect(mapStateToProps, mapActionsToProps)(AnnouncementDetail);