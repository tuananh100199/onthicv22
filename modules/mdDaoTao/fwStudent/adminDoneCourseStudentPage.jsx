...
const defaultTitleHoanThanhKhoaHoc = 'Thông báo hoàn thành khóa học',
...
class NotificationModal extends AdminModal {
    ...
    onShow = (item) => {
        ...
        newContent = '';
        if(item && item.list){
            newAbstract = abstract.replaceAll('{khoa}', item.course && item.course.name);
            newContent = content.replaceAll('{khoa}', item.course && item.course.name);
        } else {
    ...
    render = () => this.renderModal({
        ...
        body: <>
            ...
            <FormEditor ref={e => this.itemContent = e} height='250px' smallText={'{ho_ten},{cmnd}'} uploadUrl='/user/upload?category=notification' label='Nội dung' readOnly={this.props.readOnly} />
        </>,
        buttons:
...
class FailGraduationPage extends AdminPage {
    state = { searchText: '' };
    componentDidMount() {
        T.ready('/user/student/done-course', () => {
            T.showSearchBox();
            ...
            this.props.getNotificationTemplateAll({}, data => {
                if (data && data.length) {
                    ...
                    if (indexHoanThanhKhoaHoc != -1) {
                        this.setState({ data: data[indexHoanThanhKhoaHoc] });
                    } else {
                        this.setState({
                            data: {
                                title: defaultTitleHoanThanhKhoaHoc,
                                abstract: defaultAbstractHoanThanhKhoaHoc,
                                content: defaultContentHoanThanhKhoaHoc
                            }
                        });
                    }
                } else {
    ...
    onSearch = ({ pageNumber, pageSize, searchText = this.state.searchText, course = this.course.value() }, done) => {
        ...
        const condition = course == '0' ? { searchText, courseType, datSatHach:true } : { searchText, course, datSatHach: true };
    ...
    render() {
        ...
        return this.renderPage({
            ...
            breadcrumb: ['Học viên hoàn thành khóa học'],
            content: <>
                <div className='tile'>
                ...
                {this.course && (this.course.value() != '0') ? <CirclePageButton type='custom' customClassName='btn-warning' customIcon='fa-paper-plane' style={{ right: '20px' }} onClick={(e) => this.sendNotificationCourse(e, this.course.value())} /> : null}
