...
class ImportPage extends AdminPage {
    ...
    onChange = (e, text = undefined) => {
        ...
        const params = ['startRow', 'endRow', 'fullnameCol', 'birthdayCol', 'courseCol', 'courseTypeCol', 'courseType', 'idCardCol','kySatHach'],
            isFileBoxHide = params.some(item => [null, ''].includes(this[item].value()));
        this.setState({ isFileBoxHide,kySatHach:this.kySatHach.value() }, () => {
    ...
    save = () => {
        if (this.state.data && this.state.data.length > 0) {
            this.props.importFailPassStudent(this.state.data.map(({ identityCard, course }) => ({ identityCard, course,kySatHach:this.state.kySatHach })), this.importType.value(), () => this.setState({ data: [], isFileBoxHide: false }));
            ...
            content: <>
                < div className='tile' style={{ display: this.state.data && this.state.data.length > 0 ? 'none':'block' }}>
                    ...
                    <div className='row'>
                        ...
                        <FormSelect ref={e => this.kySatHach = e} onChange={e => this.onChange(e)} label='kỳ sát hạch' className='col-md-4' data={ajaxSelectLicenseTest} />
                    </div>
                </div>
