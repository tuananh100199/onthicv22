module.exports = app => {
    ...
    app.post('/api/payment', app.permission.check(), (req, res) => { 
        const suffixKeys= ['smsAPIToken', 'moneyLine', 'moneyStr', 'contentLine', 'contentStr']; //'moneyStartStr', 'moneyEndStr', 'contentStartStr', 'contentEndStr'
        ...
        app.redis.mget(keys, (error, values) => {
            ...
            const BANK=['OCB'], moneyStartStr = values[2].split('/:money/')[0], moneyEndStr = values[2].split('/:money/')[1],  
            ...
            if(originatingAddress && BANK.includes(originatingAddress.toUpperCase()) && body && body.includes(moneyStartStr)){ // && body.includes(HIEPPHAT)
                ...
                if(lines && lines.length > 0){
                    ...
                    if(contentLine){ //42354769 A1
                        ...
                        if(contentPart){
                            ...
                            contentPart = contentPart.match(/.*(\b(\d{9}|\d{12})\b \b\w+\b).*/);
                            if(contentPart){
