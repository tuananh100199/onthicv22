        ...
        return (
            <div className='row'>
                <div className='col-md-3' >
                    ...
                    {feedback.length ?feedback.map((item, index) =><div key={index} style={{ marginTop: 8, marginRight: 8,...(item._id==this.state._feedbackIdSelected && { backgroundColor:'#428bca' }) }}>              
                    <a href='#' style={{ color: item._id==this.state._feedbackIdSelected ?'white': 'black' }} onClick={e => this.onClick(e, item._id)}>{`${item.user && item.user.lastname} ${item.user && item.user.firstname}`}</a>
                        </div>)  :'Danh sách trống!'}
                    </div>
                </div>
