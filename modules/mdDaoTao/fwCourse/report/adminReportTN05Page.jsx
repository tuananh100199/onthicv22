...
import { exportTN05, getCourse, updateCourse } from '../redux';
...
class HoiDongTotNghiepModal extends AdminModal {
    ...
    onShow = (item) => {
        const { _id, name, chucVu, nhiemVu, gender, chamLyThuyet, chamThucHanh, isTruongBanChamThi, isThuKyChamThi  } = item || { _id: null, name: '', chucVu: '', nhiemVu:'', gender:'male', chamLyThuyet: false, chamThucHanh: false, isTruongBanChamThi:false, isThuKyChamThi: false  };
        ...
        this.itemTruongBanChamThi.value(isTruongBanChamThi || false);
        this.itemThuKyChamThi.value(isThuKyChamThi || false);
    ...
    onSubmit = () => {
        ...
        const changes = {
            ...
            isTruongBanChamThi: this.itemTruongBanChamThi.value(),
            isThuKyChamThi: this.itemThuKyChamThi.value(),
        };
        if(_id){
    ...
    render = () => this.renderModal({
        ...
        body: (
            <div className='row'>
                ...
                <FormCheckbox ref={e => this.itemTruongBanChamThi = e} className='col-md-6' label='Trưởng ban' readOnly={false} ></FormCheckbox>
                <FormCheckbox ref={e => this.itemThuKyChamThi = e} className='col-md-6' label='Thư ký' readOnly={false} ></FormCheckbox>
...
class AdminReportTN05Page extends AdminPage {
    state = { searchText: '' };
    componentDidMount() {
        T.ready('/user/course', () => {
            const params = T.routeMatcher('/user/course/:_id/report/ban-coi-cham-thi').parse(window.location.pathname);
    ...
    exportTN05 = () => {
       ...
       if(courseId){
            this.props.exportTN05(courseId, (data) => {
                FileSaver.saveAs(new Blob([new Uint8Array(data.buf.data)]), 'QĐ Ban Coi Cham Thi.docx');
            });
        } else{
    ...
    render() {
        ...
        return this.renderPage({
            ...
            title: 'Ban coi chấm thi (Phụ lục TN05)',
            breadcrumb: [<Link key={0} to='/user/course'>Khóa học</Link>,<Link key={1} to={'/user/course/' + this.state.courseId + '/report'}>Báo cáo</Link>,'Ban coi chấm thi (Phụ lục TN05)'],
            content: <>
                <div className='tile'>
                    <h3 className='tile-title col-md-12' style={{ paddingLeft: 15, marginBottom: 5 }}>Thành viên hội đồng</h3>
                    {table}
                </div>
                <CirclePageButton type='export' onClick={() => this.exportTN05()} />
...
const mapActionsToProps = { getStudentPage, updateStudent, getCourseTypeAll, exportTN05, getCourse, updateCourse };
export default connect(mapStateToProps, mapActionsToProps)(AdminReportTN05Page);
