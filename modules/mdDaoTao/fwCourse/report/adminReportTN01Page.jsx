...
import { exportTN01, getCourse, updateCourse } from '../redux';
...
class AdminReportTN01Page extends AdminPage {
    state = { searchText: '' };
    componentDidMount() {
        T.ready('/user/course', () => {
            const params = T.routeMatcher('/user/course/:_id/report/bien-ban-du-thi-tot-nghiep').parse(window.location.pathname);
    ...
    exportTN01 = () => {
        ...
        const bienBan = {
            numberOfRegister: this.numberOfRegister.value(),
            numberOfStudent: this.numberOfStudent.value(),
        };
        if(!bienBan.numberOfRegister){
            T.notify('Số lượng học viên đăng ký không được trống!', 'danger');
        } else if(!bienBan.numberOfStudent){
            T.notify('Số lượng học viên đạt không được trống!', 'danger');
        } else if(courseId){
            this.props.exportTN01(courseId, bienBan, (data) => {
                FileSaver.saveAs(new Blob([new Uint8Array(data.buf.data)]), 'Bien Ban Xet Du Thi Tot Nghiep.docx');
            });
        } else{
    ...
    render() {
        ...
        return this.renderPage({
            ...
            title: 'Biên bản dự thi tốt nghiệp (Phụ lục TN01)',
            breadcrumb: [<Link key={0} to='/user/course'>Khóa học</Link>,<Link key={1} to={'/user/course/' + this.state.courseId + '/report'}>Báo cáo</Link>,'Biên bản dự thi tốt nghiệp (Phụ lục TN01)'],
            content: <>
                <div className='tile'>
                    <div className='row'>
                        <FormTextBox type='number' ref={e => this.numberOfRegister = e} className={'col-md-3'} label='Tổng số danh sách học viên' readOnly={false} />   
                        <FormTextBox type='number' ref={e => this.numberOfStudent = e} className={'col-md-3'} label='Tổng số học viên đủ điều kiện dự thi' readOnly={false} />     
                    </div>
                ...
                <CirclePageButton type='export' onClick={() => this.exportTN01()} />
...
const mapActionsToProps = { getStudentPage, updateStudent, getCourseTypeAll, exportTN01, getCourse, updateCourse };
export default connect(mapStateToProps, mapActionsToProps)(AdminReportTN01Page);
