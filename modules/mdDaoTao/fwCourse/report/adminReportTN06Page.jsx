...
class AdminReportTN06Page extends AdminPage {
    state = { searchText: '' };
    componentDidMount() {
        T.ready('/user/course', () => {
            const params = T.routeMatcher('/user/course/:_id/report/danh-sach-hoc-vien-thi-tot-nghiep').parse(window.location.pathname);
    ...
    render() {
        ...
        return this.renderPage({
            ...
            title: 'Danh sách học viên dự thi tốt nghiệp (Phụ lục TN06)',
            breadcrumb: [<Link key={0} to='/user/course'>Khóa học</Link>,<Link key={1} to={'/user/course/' + this.state.courseId + '/report'}>Báo cáo</Link>,'Danh sách học viên dự thi tốt nghiệp (Phụ lục TN06)'],
            content: <>
                <div className='tile'>
                ...
                <CirclePageButton type='export' onClick={() => this.exportTN06()} />
