...
import { exportSH02, getCourse } from '../redux';
...
class AdminReportSH02Page extends AdminPage {
    state = { searchText: '' };
    componentDidMount() {
        T.ready('/user/course', () => {
            const params = T.routeMatcher('/user/course/:_id/report/bang-diem-thi-sat-hach').parse(window.location.pathname);
    ...
    exportSH02 = () => {
        ...
        if(list && list.length){
            this.props.exportSH02(listId, (data) => {
                FileSaver.saveAs(new Blob([new Uint8Array(data.buf.data)]), 'Phu_Luc_SH02.docx');
            });
        } else{
    ...
    render() {
        ...
        return this.renderPage({
            ...
            title: 'Bảng điểm thi sát hạch (Phụ lục SH02)',
            breadcrumb: [<Link key={0} to='/user/course'>Khóa học</Link>,<Link key={1} to={'/user/course/' + this.state.courseId + '/report'}>Báo cáo</Link>,'Bảng điểm thi sát hạch (Phụ lục SH02)'],
            content: <>
                <div className='tile'>
                ...
                <CirclePageButton type='export' onClick={() => this.exportSH02()} />
...
const mapActionsToProps = { getStudentPage, updateStudent, getCourseTypeAll, exportSH02, getCourse };
export default connect(mapStateToProps, mapActionsToProps)(AdminReportSH02Page);
