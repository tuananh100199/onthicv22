...
import { exportTN02, getCourse } from '../redux';
...
class AdminReportTN02Page extends AdminPage {
    state = { searchText: '' };
    componentDidMount() {
        T.ready('/user/course', () => {
            const params = T.routeMatcher('/user/course/:_id/report/danh-sach-du-thi-tot-nghiep').parse(window.location.pathname);
    ...
    exportTN02 = () => {
        ...
        if(list && list.length){
            this.props.exportTN02(listId, (data) => {
                FileSaver.saveAs(new Blob([new Uint8Array(data.buf.data)]), 'Phu_Luc_TN02.docx');
            });
        } else{
    ...
    render() {
        ...
        return this.renderPage({
            ...
            title: 'Danh sách học viên xét dự thi tốt nghiệp (Phụ lục TN02)',
            breadcrumb: [<Link key={0} to='/user/course'>Khóa học</Link>,<Link key={1} to={'/user/course/' + this.state.courseId + '/report'}>Báo cáo</Link>,'Danh sách học viên xét dự thi tốt nghiệp (Phụ lục TN02)'],
            content: <>
                <div className='tile'>
                ...
                <CirclePageButton type='export' onClick={() => this.exportTN02()} />
...
const mapActionsToProps = { getStudentPage, updateStudent, getCourseTypeAll, exportTN02, getCourse };
export default connect(mapStateToProps, mapActionsToProps)(AdminReportTN02Page);
