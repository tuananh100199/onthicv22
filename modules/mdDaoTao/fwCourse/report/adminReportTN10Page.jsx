...
import { exportTN10, getCourse } from '../redux';
...
class AdminReportTN10Page extends AdminPage {
    state = { searchText: '' };
    componentDidMount() {
        T.ready('/user/course', () => {
            const params = T.routeMatcher('/user/course/:_id/report/bang-diem-thi-tot-nghiep').parse(window.location.pathname);
    ...
    exportTN10 = () => {
        ...
        if(list && list.length){
            this.props.exportTN10(listId, (data) => {
                FileSaver.saveAs(new Blob([new Uint8Array(data.buf.data)]), 'Phu_Luc_TN10.docx');
            });
        } else{
        ...
        const table = renderTable({
            ...
            renderHead: () => (
                <tr>
                    ...
                    <th style={{ width: 'auto', textAlign: 'center' }} nowrap='true'>TH Đường</th>
                    <th style={{ width: 'auto', textAlign: 'center' }} nowrap='true'>TH Chữ Chi</th>
            ...
            renderRow: (item, index) => (
                <tr key={index}>
                    ...
                    <TableCell content={28} />
                    <TableCell content={80} />
                    <TableCell content={95} />
                    <TableCell content={16} />
                    <TableCell type='buttons' content={item} permission={permission} onEdit={this.edit} onDelete={this.delete} />
                </tr>)
        });
        return this.renderPage({
            icon: 'fa fa-users', // select icon
            title: 'Bảng điểm thi tốt nghiệp (Phụ lục TN10)',
            breadcrumb: [<Link key={0} to='/user/course'>Khóa học</Link>,<Link key={1} to={'/user/course/' + this.state.courseId + '/report'}>Báo cáo</Link>,'Bảng điểm thi tốt nghiệp (Phụ lục TN10)'],
            content: <>
                <div className='tile'>
                ...
                <CirclePageButton type='export' onClick={() => this.exportTN10()} />
...
const mapActionsToProps = { getStudentPage, updateStudent, getCourseTypeAll, exportTN10, getCourse };
export default connect(mapStateToProps, mapActionsToProps)(AdminReportTN10Page);
