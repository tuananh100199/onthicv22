...
import { getStudyProgramItem, updateStudyProgram } from './redux';
import { Link } from 'react-router-dom';
import { AdminPage, FormTextBox, FormRichTextBox, FormEditor, FormCheckbox } from 'view/component/AdminPage';

class StudyProgramEditPage extends AdminPage {
    state = {};
    componentDidMount() {
        T.ready('/user/study-program', () => {
            const route = T.routeMatcher('/user/study-program/:_id'), params = route.parse(window.location.pathname);
            this.props.getStudyProgramItem(params._id, data => {
                if (data.item) {
                    ...
                    this.itemImage.setData('study-program:' + (_id || 'new'), image ? image : '/img/avatar.png');
    ...
    save = () => {
        ...
        if (!changes.title) {
            ...
            this.props.updateStudyProgram(this.state._id, changes, () => T.notify('Cập nhật cơ sở thành công!', 'success'));
        }
    }
    ...
    render() {
        ...
        return this.renderPage({
            ...
            breadcrumb: [<Link key={0} to='/user/study-program'>Cơ sở đào tạo</Link>, 'Chỉnh sửa'],
            content: <>
                <div className='tile'>
...
const mapActionsToProps = { updateStudyProgram, getStudyProgramItem };
export default connect(mapStateToProps, mapActionsToProps)(StudyProgramEditPage);
