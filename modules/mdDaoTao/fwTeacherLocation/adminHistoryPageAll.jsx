...
class TeacherHistoryPage extends AdminPage {
    componentDidMount() {
        T.ready('/user/course', () => {
            const params = T.routeMatcher('/user/course/:_id/location-course').parse(window.location.pathname);
            const course = this.props.course ? this.props.course.item : null;
            let list = [];
            if (course) {
                const teacherGroups = course.teacherGroups;
                teacherGroups.forEach(group => {
                    list.push(group.teacher._id);
                });
                this.props.getTeacherLocationPage(1,50, {teacher: {$in: list}});
            } else {
                this.props.getCourse(params._id, data => {
                    if (data.error) {
                        ...
                        console.log(data);
                        const teacherGroups = data.item && data.item.teacherGroups;
                        teacherGroups.forEach(group => {
                            list.push(group.teacher._id);
                        });
                        this.props.getTeacherLocationPage(1,50, {teacher: {$in: list}});
                ...
                table = renderTable({
                    ...
                    renderHead: () => (
                        <TableHead getPage = {this.props.getTeacherLocationPage}>
                            ...
                            <TableHeadCell name='fullName' content='Giáo viên' style={{width:'100%'}}  />
                            <TableHeadCell name='fullName' content='Học viên' style={{width:'auto' , textAlign: 'center'}} nowrap='true'  />
                    ...
                    renderRow: (item, index) => (
                        <tr key={index}>
                            ...
                            <TableCell type='text' content={item.teacher ? item.teacher.lastname + ' ' + item.teacher.firstname : ''} />
                            <TableCell type='text' style={{ width: 'auto', textAlign: 'center', whiteSpace: 'nowrap' }} content={item.timeTable && item.timeTable.student ? item.timeTable.student.lastname + ' ' + item.timeTable.student.firstname : ''} />
