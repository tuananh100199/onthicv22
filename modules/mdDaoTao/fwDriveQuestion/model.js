module.exports = app => {
    const schema = app.database.mongoDB.Schema({
        ...
        importance: { type: Boolean, default: false },                              // true => câu quan trọng dễ sai
        ...
        getPage: (pageNumber, pageSize, condition, done) => {
            model.countDocuments(condition, (error, totalItem) => {
                if (error) {
                    ...
                    model.find(condition).populate('categories', 'title').sort({ priority: -1 }).skip(skipNumber).limit(result.pageSize).exec((error, items) => {
